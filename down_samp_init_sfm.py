...
for i in range(len(folder_path_each)):
    ...
    if os.path.isfile(path):
        ...
        bin_path = (
            data_path
            + sub_folder_path_each_1[i]
            + "/gaussian_splat/sparse/0/points3D.bin"
        )
    ...
    idx = np.random.randint(
        0, len(plydata.elements[0]["x"]), size=reso_Gaussian**2
    )  # 16384 for 128*128; 262144 for 512*512
    ...
    PlyData([el]).write(
        data_path
        + sub_folder_path_each_1[i]
        + f"/gaussian_splat/sparse/0/points3D_{reso_Gaussian}.ply"
    )

    # run Gaussian Splatting on this scene
    bash_script = (
        f"python train.py -s {data_path}{sub_folder_path_each_1[i]}/gaussian_splat/"
    )
    print(bash_script)
