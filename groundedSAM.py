...
camera_path = "your/path/to/DL3DV-10K/estimated_camera_parameters/"  # only if you save SfM results to /estimated_camera_parameters folder seperately
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        cam_info = CameraInfo(
            ...
            image=dummy_image,
            image_path="",
            image_name=f"frame_{extr.camera_id:4d}",
            width=width,
            height=height,
        )
...
for i in tqdm(range(len(folder_path_each))):
    ...
    scene_info = readColmapSceneInfo(
        camera_path + "camera_" + folder_path_each[i], dummy_image_path, eval=False
    )
    ...
    gs_params_path_each = os.path.join(
        data_path + folder_path_each[i] + "/point_cloud/iteration_30000/",
        "point_cloud_316.ply",
    )  # this point_cloud_316.ply is the output of 3DGS optimization, change prefix "_316" to your actual output name.
    ...
    gs_to_pixel = torch.mm(
        torch.tensor(train_cameras[camera_id_vis].full_proj_transform).T,
        gs_xyz_persp.T.cuda(),
    ).cpu()  # K*[R,T]*[x,y,z,1]^T
    ...
    neighbor_n = pynanoflann.KDTree(
        n_neighbors=40, metric="L2", radius=0.1 * scale_radius
    )
    neighbor_n.fit(xyz)
    ...
    for ik in range(int(num_k)):
        if len(queue_k) < num_k:
            ...
            elmts_to_be_added = neighbor_idx[
                np.array(queue_k, dtype=np.int64)[pick_rdm_idx], :
            ].flatten()
    ...
    translate, scale_factor = get_tf_cams(
        xyz_norm, target_radius=10.0
    )  # target_radius = any value you want
    xyz_norm = (xyz_norm + translate) * scale_factor
    scale_norm = scale_norm + np.log(scale_factor)
    gs_full_params = np.concatenate(
        (xyz_norm, color_rgb, opacity[:, None], scale_norm, rot), axis=1
    )
    ...
    f_dc = (
        torch.tensor(gs_full_params[:, 3:6][:, None, :])
        .transpose(1, 2)
        .flatten(start_dim=1)
        .contiguous()
        .cpu()
    ...
    f_rest = (
        torch.zeros([gs_full_params.shape[0], 0, 3])
        .transpose(1, 2)
        .flatten(start_dim=1)
        .contiguous()
        .cpu()
    ...
    print(
        f"{i}/{len(folder_path_each)} Semantic-filtering and normalization finished!!!"
    )
